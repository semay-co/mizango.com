...
const AnalyticsContent = ({
  ...
  useEffect(() => {
    ...
    const data = chartQuery.data.map((doc: any) => {
      return {
        time: doc.createdAt,
        tick: getTick(doc.createdAt),
        type: doc.dataCache?.vehicle?.type,
        vehicle: doc.vehicleId,
        weights: doc.weights,
        isMistake: doc.isMistake,
      }
    })

    const dataByHour = Object.values(
      chartQuery.data.reduce((acc: any, doc: any) => {
        if (doc.isMistake) {
          return acc
        }

        const tick = getTick(doc.createdAt)
        const type = doc.dataCache?.vehicle?.type
        ...
        acc[tick][`revenue${type}`] += typePrice[type]
        acc[tick][`records${type}`]++
        // acc[hour][`sessions${type}`]++

        // if (doc.weights.length > 0) {
        //   const h = moment(+doc.weights[1]?.createdAt).format('hh A')
        //   if (h !== hour) {
        //     acc[h] = acc[h] || init
        //     acc[h][`sessions${type}`]++
        //   }
    ...
    const dataByVehicle = chartQuery.data.reduce(
      (acc: any, doc: any) => {
        if (doc.isMistake) {
          return acc
        }
        ...
        acc[type][`records`]++
        acc[type][`revenue`] += typePrice[type]
    ...
    setRecords(data)
    setByHour(dataByHour)
    setByVehicle(dataByVehicle)
  }, [chartQuery.data])

  useEffect(() => {
    const filtered = records.filter((record: any) => !record.isMistake)

    const value = {
      revenue: filtered.reduce(
        (acc, curr: any) => acc + typePrice[curr.type],
        0
      ),
      records: filtered.length,
      // sessions: filtered.reduce((a: any[], c: any) => {
      //   a.push(...c.weights)
      //   return a
      // }, []).length,
    }
  ...
  return (
    <Card>
      <code>{JSON.stringify(chartQuery.data)}</code>
      <CardHeader className='flex sm:flex-row flex-col items-stretch space-y-0 p-0 border-b'>
        <div className='flex'>
          {['revenue', 'records'].map((key) => {
            const chart = key as keyof typeof chartConfig
